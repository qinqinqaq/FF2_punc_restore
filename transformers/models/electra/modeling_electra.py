...
class ElectraSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # todo
        # self.talk_matrix = torch.nn.Parameter(torch.Tensor(torch.randn(self.num_attention_heads, self.num_attention_heads)))
        self.talk_matrix = torch.nn.Parameter(torch.Tensor(torch.normal(0, 0.1 / (1024 ** 0.5), (self.num_attention_heads, self.num_attention_heads))))
    ...
    def forward(
        ...
        # todo
        attention_scores = torch.einsum('abcd,eb->abcd', attention_scores, self.talk_matrix) + attention_scores
...
class ElectraEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        # todo
        # self.layer = nn.ModuleList([ElectraLayer(config) for _ in range(config.num_hidden_layers-4)])
